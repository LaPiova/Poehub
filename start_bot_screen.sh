...
if screen -list | grep -q "poebot"; then
    echo -e "${YELLOW}⚠${NC}  Screen session 'poebot' already exists!"
    echo ""
    echo "Options:"
    echo "  1. Attach to existing session: screen -r poebot"
    echo "  2. Kill and restart: screen -X -S poebot quit && $0"
    echo ""
    exit 1
fi
