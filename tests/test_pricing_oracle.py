...
class TestPricingOracle:
    ...
    def test_get_price_default_poe(self):
        in_price, out_price, currency = PricingOracle.get_price("poe", "unknown-model-123")
        assert currency == "Points"
        assert in_price == 200.0 # Default

    def test_get_price_default_usd(self):
        in_price, out_price, currency = PricingOracle.get_price("openai", "unknown-model")
        assert currency == "USD"
        assert in_price == 0.0

    def test_calculate_cost_usd(self):
        usage = TokenUsage(input_tokens=1_000_000, output_tokens=1_000_000, currency="USD")
    ...
    def test_calculate_cost_points_precalculated(self):
        ...
        usage = TokenUsage(10, 10, cost=500.0, currency="Points")
    ...
    def test_calculate_cost_points_fallback(self):
        ...
        usage = TokenUsage(100, 100, currency="USD") # passed as USD usually
        ...
        assert cost == 0.0289 # Testing existing logic, even if it seems odd for points.
