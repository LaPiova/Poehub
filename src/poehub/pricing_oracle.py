...
class TokenUsage:
    ...
    output_tokens: int
    cost: float = 0.0
    currency: str = "USD"  # "USD" or "Points"
...
class PricingOracle:
    ...
    @classmethod
    def update_rate(cls, provider: str, model: str, in_price: float, out_price: float, currency: str = "USD") -> None:
    ...
    def get_price(cls, provider: str, model: str) -> Tuple[float, float, str]:
        ...
        for k, v in cls.RATES.items():
            if k.split("/")[1] == model.lower():
                 if provider == "poe": return (*v, "Points")
                 return (*v, "USD")
