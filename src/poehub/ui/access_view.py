...
class AccessControlView(discord.ui.View):
    ...
    async def update_view(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="🛡️ Access Control & Budget Manager",
        ...
        if self.active_guild:
            ...
            embed.add_field(name="💰 Standard Budget (USD)", value=f"Limit: **{usd_str}**\nSpent: **${spend_usd:.2f}**", inline=True)
            embed.add_field(name="🔮 Poe Budget (Points)", value=f"Limit: **{pts_str}**\nSpent: **{spend_pts:,.0f}**", inline=True)
...
class ToggleAccessButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not view.active_guild: return
...
class SetLimitButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        view: AccessControlView = self.view
        if not view.active_guild: return
        await interaction.response.send_modal(BudgetLimitModal(self.cog, view.active_guild, view))
...
class ResetSpendButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await view.update_view(interaction)
        await interaction.followup.send(f"Status: Spend usage (USD & Points) reset for {view.active_guild.name}", ephemeral=True)


class BudgetLimitModal(discord.ui.Modal):
    def __init__(self, cog: "PoeHub", guild: discord.Guild, parent_view: AccessControlView):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not usd_val:
            ...
            try:
                ...
                await interaction.response.send_message("❌ Invalid USD value.", ephemeral=True)
                return
        ...
        if not pts_val:
            ...
            try:
                ...
                if limit_p < 0: raise ValueError
                await self.cog.config.guild(self.guild).monthly_limit_points.set(limit_p)
